...
creates a worktree-local `.env` from tracked `.env.example`, replaces only local test-safe values
(`SECRET_KEY` and `DB_HOST`), and installs the repository's shared pre-commit hook. It refuses an
unsafe name, an existing target, or a missing root virtual environment before changing Git state.
It never reads or links the root `.env`.
...
- `make hooks` installs or repairs the shared pre-commit hook for existing checkouts.
...
linking, hook installation, Git command construction, and Makefile contracts. A real smoke creates
a temporary worktree through the public command and proves `.venv/bin/pytest --version` plus
`manage.py check` under CI-like settings, then removes the smoke worktree and branch through normal
Git commands.
