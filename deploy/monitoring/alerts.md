# FindMe Photo staging monitoring alert manifest

Create these seven alerts in Yandex Monitoring after the dashboard and email notification channel
exist. Every selector is restricted to `service=custom` and the production-like probe selector is
`folderId="__YANDEX_CLOUD_FOLDER_ID__",environment=staging,check=canonical-health`;
`environment=validation` is never selected.

Notification channel: the one staging operator **email** channel created during activation.
...
- Resource name: `findme-photo-staging-public-service-unavailable`
- Selector: `findme_probe_success{folderId="__YANDEX_CLOUD_FOLDER_ID__", service="custom", environment="staging", check="canonical-health"}`.
...
- Notification channel: staging operator email.
- Firing notification: `FindMe staging public health has two failed or missing probe points; inspect the external check and dashboard.`
- Recovery notification: `FindMe staging public health probe has recovered.`

## TLS certificate expiring

- Resource name: `findme-photo-staging-tls-certificate-expiring`
- Selector: `findme_probe_tls_days_remaining{folderId="__YANDEX_CLOUD_FOLDER_ID__", service="custom", environment="staging", check="canonical-health"}`.
...
- Notification channel: staging operator email.
- Firing notification: `FindMe staging TLS certificate has fewer than 14 days remaining.`
- Recovery notification: `FindMe staging TLS certificate lifetime is back above 14 days.`

## VM telemetry missing

- Resource name: `findme-photo-staging-vm-telemetry-missing`
...
- Notification channel: staging operator email.
- Firing notification: `FindMe staging VM telemetry has been missing for five minutes; public health may still be up.`
- Recovery notification: `FindMe staging VM telemetry has resumed.`

## Disk space critical

- Resource name: `findme-photo-staging-disk-space-critical`
...
- Notification channel: staging operator email.
- Firing notification: `FindMe staging system filesystem is below 10% free or 5 GiB available.`
- Recovery notification: `FindMe staging system filesystem capacity has recovered.`

## Memory pressure

- Resource name: `findme-photo-staging-memory-pressure`
...
- No data: do not fire this resource-pressure alert; missing telemetry is handled by the telemetry alert.
- Notification channel: staging operator email.
- Firing notification: `FindMe staging available memory has been below 10% for 15 minutes.`
- Recovery notification: `FindMe staging available memory has recovered above 10%.`

## CPU pressure

- Resource name: `findme-photo-staging-cpu-pressure`
...
- No data: do not fire this resource-pressure alert; missing telemetry is handled by the telemetry alert.
- Notification channel: staging operator email.
- Firing notification: `FindMe staging CPU utilization has been above 90% for 15 minutes.`
- Recovery notification: `FindMe staging CPU utilization has recovered below 90%.`

## Application 5xx degradation

- Resource name: `findme-photo-staging-application-5xx-degradation`
- Selector: `app.findme_http_requests_total{folderId="__YANDEX_CLOUD_FOLDER_ID__", service="custom", environment="staging"}` and the same
...
- Notification channel: staging operator email.
- Firing notification: `FindMe staging HTTP 5xx responses exceed 20% with at least five requests in five minutes.`
- Recovery notification: `FindMe staging HTTP 5xx response rate has recovered.`
