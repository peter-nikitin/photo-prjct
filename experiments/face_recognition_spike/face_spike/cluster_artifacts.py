            ...
            rows.append(
                {
                    ...
                    "minimum_side_px": _number(face.quality.minimum_side_px),
                    "relative_area": _number(face.quality.relative_area),
                    "sharpness": _number(face.quality.sharpness),
                    "quality_decision": face.quality.decision,
                    "quality_reasons": "|".join(face.quality.reasons),
                    "status": face.status,
...
def _build_metrics(
    ...
    return {
        ...
        "quality_rejection_reasons": dict(
            sorted(Counter(reason for face in faces for reason in face.quality.reasons).items())
        ),
