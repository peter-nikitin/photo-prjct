...
class SyncFeatureFlagsTests(TestCase):
    ...
    def test_creates_every_registered_definition_off_with_sanitized_counts(self) -> None:
        ...
        self.assertEqual(
            output,
            "Feature flags synchronized: created=7 updated=0 preserved=0 deleted=0.\n",
        )
    ...
    def test_preserves_registered_states_and_creation_timestamp(self) -> None:
        ...
        self.assertEqual(
            output,
            "Feature flags synchronized: created=4 updated=0 preserved=3 deleted=0.\n",
        )
    ...
    def test_refreshes_description_without_changing_state_or_creation_timestamp(self) -> None:
        ...
        self.assertEqual(
            output,
            "Feature flags synchronized: created=6 updated=1 preserved=0 deleted=0.\n",
        )
    ...
    def test_deletes_stale_rows_and_is_idempotent(self) -> None:
        ...
        self.assertEqual(
            self.sync(),
            "Feature flags synchronized: created=7 updated=0 preserved=0 deleted=1.\n",
        )
        self.assertFalse(FeatureFlag.objects.filter(key="retired-release").exists())
        self.assertEqual(
            self.sync(),
            "Feature flags synchronized: created=0 updated=0 preserved=7 deleted=0.\n",
        )
