...
- imgproxy receives S3 static credentials belonging to a dedicated service account. The bucket policy grants only `s3:GetObject` on `arn:aws:s3:::${PRIVATE_MEDIA_S3_BUCKET}/derivatives/previews/*`; it grants no bucket listing or mutation. Existing policy is read, normalized, and merged with this single statement rather than replaced blindly.
- Secrets are distinct: `GALLERY_CDN_TOKEN_SECRET`, `GALLERY_IMGPROXY_KEY`, `GALLERY_IMGPROXY_SALT`, `IMAGE_ORIGIN_HEADER_SECRET`, `IMAGE_ORIGIN_S3_ACCESS_KEY_ID`, and `IMAGE_ORIGIN_S3_SECRET_ACCESS_KEY`. The application receives only the first three; the origin receives only the latter five excluding the CDN token. Provisioning receives only the CDN token and origin-header secret.
...
- [ ] Add failing contract tests for stable cloud/folder discovery, exact resource names, VM shape, reserved address, security-group ports, dedicated service account, prefix-only policy statement, secret consumer separation, masked command output, and refusal to mutate without `--apply` plus an exact approval nonce.
...
- [ ] Generate a bucket-policy patch that adds one principal-scoped `s3:GetObject` statement for `arn:aws:s3:::${PRIVATE_MEDIA_S3_BUCKET}/derivatives/previews/*`. Resolve the bucket from the canonical deployment's `PRIVATE_MEDIA_S3_BUCKET`, preserve every unrelated existing statement, and require a normalized before/after diff. Add a credential probe that must allow one accepted preview read and deny `ListBucket`, original/staging reads, put, multipart, ACL, and delete.
