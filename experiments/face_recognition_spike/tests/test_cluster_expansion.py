...
def test_evaluation_separates_calibration_from_held_out_metrics_and_preserves_identities() -> None:
    ...
    report = evaluate_cluster_expansion(benchmark, index, clusters, _configuration())
...
def test_report_identity_is_deterministic_and_does_not_include_query_or_photo_artifacts() -> None:
    ...
    first = evaluate_cluster_expansion(benchmark, index, clusters, _configuration())
    second = evaluate_cluster_expansion(benchmark, index, clusters, _configuration())
...
def test_report_binds_resource_evidence_but_not_nondeterministic_measurements() -> None:
    ...
    missing = evaluate_cluster_expansion(benchmark, index, clusters, _configuration())
...
def test_report_identity_excludes_volatile_manifest_hashes_and_binds_stable_corpus_inputs() -> None:
    ...
    assert (
        first.report_sha256
        != evaluate_cluster_expansion(
            benchmark, index, changed_membership, _configuration()
        ).report_sha256
    )
    assert (
        first.report_sha256
        != evaluate_cluster_expansion(
            changed_model_benchmark, changed_model_index, clusters, _configuration()
        ).report_sha256
    )
...
def test_report_identity_binds_metric_preserving_normalized_embedding_matrix() -> None:
    ...
    first = evaluate_cluster_expansion(benchmark, index, clusters, _configuration())
    perturbed = evaluate_cluster_expansion(benchmark, perturbed_index, clusters, _configuration())
