...
2. Confirm the goal, scope boundaries, acceptance criteria, compatibility requirements, and
   operational constraints. Stop for user input only when a missing choice materially changes the
   outcome.
3. Detect architectural decisions hidden in the request. Invoke `$write-adr` before planning when
...
4. Copy `docs/plans/0000-template.md` to `docs/plans/YYYY-MM-DD-topic.md`. Link exact architecture
   sections and ADRs; write `none` only after checking.
5. Decompose work into independently verifiable tasks. Name exact paths, interfaces, data flow,
...
6. Specify observable acceptance criteria and exact verification commands. Include expected
   outcomes rather than vague instructions such as “test thoroughly.”
7. Describe configuration, deployment order, compatibility, monitoring, and rollback. State `None`
   explicitly when a section has no runtime effect.
8. Remove unresolved implementation choices. If an open question remains, mark the plan blocked or
