...
| Event cart and checkout | production | `src/backend/templates/commerce/cart.html` | `/events/<slug>/cart/` | `/__visual__/event/cart/`, `/__visual__/event/cart/empty/` | `desktop-event-cart.png`, `desktop-event-cart-empty.png`, `mobile-event-cart.png`, `mobile-event-cart-empty.png` |
| Order | production | `src/backend/templates/commerce/order.html` | `/orders/<public-number>/`, `/orders/<public-number>/return/`, and the access-grant variant | `/__visual__/order/pending/`, `/__visual__/order/paid/`, `/__visual__/order/paid/multi/`, `/__visual__/order/email-failed/` | `desktop-order-pending.png`, `desktop-order-paid.png`, `desktop-order-paid-multi.png`, `desktop-order-email-failed.png`, `mobile-order-pending.png`, `mobile-order-paid.png`, `mobile-order-paid-multi.png`, `mobile-order-email-failed.png` |
...
On 2026-08-22, checkout moved into the production cart summary. The standalone checkout template,
dialog, visual routes, and separate checkout baselines were retired. The desktop baseline covers
the permanent form in the right summary column; the 390px mobile baseline covers the same form in
normal document flow below the photos.

On 2026-08-22, the production Order screen adopted the cart's compact visual hierarchy: a low event
header, a photo table, and a separate summary column. Existing pending, paid, and email-failed
desktop/mobile baselines cover the responsive layout without adding a new route.

On 2026-08-30, production archive coverage added deterministic single-page and multi-page states.
The existing ready selfie-result route now records the multi-page `Скачать эту страницу` action and
its exact helper while `/__visual__/event/selfie-search/ready/single/` records the one-page
`Скачать все` action. The paid ready-result baseline confirms it has no archive action. The Order
fixtures use real Django `Page` values: the existing paid route records the one-page primary action
and `/__visual__/order/paid/multi/` records numbered pagination, the multi-page helper, the bright
archive primary action, and the white resend secondary action. The added desktop and 390px mobile
...
On 2026-09-12, bib-enabled production event fixtures added the independent GET bib-number search
